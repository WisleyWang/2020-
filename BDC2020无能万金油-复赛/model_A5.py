...
test = pd.read_csv('./data/R2 ATest 0711.csv')
port = pd.read_csv('./data/port_3.csv')
...
test['label'] = 0
test['label'].loc[(test['loadingOrder']=='FA712454830300')&(test['longitude']<60)] = 1
test = test.loc[test['label']==0]
del test['label']
gc.collect()
...
def get_feature(df,model='train'):
    ...
    df['distanc2taget']=np.log(distance(df.latitude.values,df.end_lat.values,df.longitude.values,df.end_long.values)+1)/1000
...
text = train.loc[(abs(train['end_lat_gap_1'])+abs(train['end_long_gap_1']))<0.25]
def get_label1(df):
    df['timestamp'] = pd.to_datetime(df['timestamp'])
    tmp = df.groupby('loadingOrder')['timestamp'].agg(['max','min']).reset_index()
    tmp.columns=['loadingOrder','time_max','time_min']
    df = df.merge(tmp,on='loadingOrder',how='left')
    df['label1'] = (df['time_max'] - df['time_min']).dt.total_seconds()//3600
    df['label2'] = (df['timestamp'] - df['time_min']).dt.total_seconds()//3600
    del df['time_max'],df['time_min']
    return df
text = get_label1(text)
'''#删掉终点港口塞港很长的数据和离开又返回
l1 = list(text[text['label1']>48].loadingOrder.value_counts().index)
train = train[~(train['loadingOrder'].isin(l1))]'''
#取出距离港口0.25在24小时内的数据将最后一条用于构造label
text1 = text[text['label2']<=480]
merge_tabel = text1[['loadingOrder','timestamp']].drop_duplicates(['loadingOrder'],keep='last').rename(columns={'timestamp':'endtime'})
train = train.merge(merge_tabel,on=['loadingOrder'],how='left')
del text,text1,merge_tabel
#train = get_hot(train)
train = get_label(train)
gc.collect()
...
def get_window(df):
    #窗口统计变化特征
    for i in [3,5,7]:
        for j in ['speed','direction']:
            df['{}_{}_max'.format(i,j)] = df.groupby(['loadingOrder','vesselMMSI'])[j].rolling(window=i).max().values
            df['{}_{}_min'.format(i,j)] = df.groupby(['loadingOrder','vesselMMSI'])[j].rolling(window=i).min().values
            df['{}_{}_mean'.format(i,j)] = df.groupby(['loadingOrder','vesselMMSI'])[j].rolling(window=i).mean().values
            df['{}_{}_median'.format(i,j)] = df.groupby(['loadingOrder','vesselMMSI'])[j].rolling(window=i).median().values
    #窗口统计变化特征
    for i in [3,5,7]:
        for j in ['mean_speed','instant_acc','speed_diff','direction_diff']:
            df['{}_{}_mean'.format(i,j)] = df.groupby(['loadingOrder','vesselMMSI'])[j].rolling(window=i).mean().values
    return df
...
train = get_window(train)
test1 = get_window(test1)
gc.collect()
...
features = [c for c in train.columns if c in ['carrierName', 'longitude', 'latitude', 'speed', 'direction', 'end_long','end_pot',
 'end_lat', 'start_long', 'start_lat', 'start_gap', 'end_gap', 'start_long_gap', 'start_lat_gap', 'end_long_gap', 'end_lat_gap', 'lat_diff',
 'lon_diff', 'speed_diff', 'direction_diff', 'diff_seconds', 'cusum_direction','direction_valc', 'end_long_gap_1',
 'end_lat_gap_1', 'start_long_gap_1', 'start_lat_gap_1', 'year','month','day','have_run_time','stop' ,'mean_speed',                                          
'3_speed_max', '3_speed_min', '3_speed_mean', '3_speed_median', '3_direction_max', '3_direction_min', '3_direction_mean', '3_direction_median',
'5_speed_max', '5_speed_min', '5_speed_mean', '5_speed_median', '5_direction_max', '5_direction_min', '5_direction_mean', '5_direction_median',
'7_speed_max', '7_speed_min', '7_speed_mean', '7_speed_median', '7_direction_max', '7_direction_min', '7_direction_mean', '7_direction_median',
'3_mean_speed_mean', '3_instant_acc_mean', '3_speed_diff_mean', '3_direction_diff_mean', '5_mean_speed_mean', '5_instant_acc_mean', '5_speed_diff_mean', 
'5_direction_diff_mean', '7_mean_speed_mean', '7_instant_acc_mean', '7_speed_diff_mean', '7_direction_diff_mean']]#cumsum_stop
print(features)
print(len(features))
gc.collect()
...
#result,clf = build_model(train, test1,pred= features,label= 'label', is_shuffle=True)
...
test3 = pd.read_csv('./data/R2 ATest 0711.csv')
...
result2.to_csv('./result/A5.csv',index=False)