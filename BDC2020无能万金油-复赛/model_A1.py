...
def get_label1(df):
    ...
    tmp = df.groupby('loadingOrder')['timestamp'].agg({'time_max':'max','time_min':'min'})
...
#将训练数据按照是否和test路由相同分为两个部分用不同特征分别预测
l1 = list(set(train.TRANSPORT_TRACE.value_counts().index) & set(test1.TRANSPORT_TRACE.value_counts().index))
train_1 = train[train['TRANSPORT_TRACE'].isin(l1)]
test_1 = test1[test1['TRANSPORT_TRACE'].isin(l1)]
test_2 = test1[~test1['TRANSPORT_TRACE'].isin(l1)]


# In[57]:


features1 = [c for c in train.columns if c in['carrierName', 'longitude', 'latitude', 'speed', 'direction', 'TRANSPORT_TRACE', 'len','end_pot', 'end_long',
 'end_lat', 'start_long', 'start_lat', 'start_gap', 'end_gap', 'start_long_gap', 'start_lat_gap', 'end_long_gap', 'end_lat_gap', 'lat_diff',
 'lon_diff', 'speed_diff', 'direction_diff', 'diff_seconds', 'cusum_direction', 'cusum_speed', 'direction_valc', 'end_long_gap_1',
 'end_lat_gap_1', 'start_long_gap_1', 'start_lat_gap_1', 'year','month','day','have_run_time']]#cumsum_stop
print(features1)
print(len(features1))
gc.collect()
...
features2 = [c for c in train.columns if c in[ 'longitude', 'latitude', 'speed', 'direction',  'len','end_pot', 'end_long',
 'end_lat', 'start_long', 'start_lat', 'start_gap', 'end_gap', 'start_long_gap', 'start_lat_gap', 'end_long_gap', 'end_lat_gap', 'lat_diff',
 'lon_diff', 'speed_diff', 'direction_diff', 'diff_seconds', 'cusum_direction', 'cusum_speed', 'direction_valc', 'end_long_gap_1',
 'end_lat_gap_1', 'start_long_gap_1', 'start_lat_gap_1','have_run_time']]#cumsum_stop
print(features2)
print(len(features2))
gc.collect()
...
def bulid_onetrain1(train_data, test,pred= features1,label= 'label',seed=1099,est=6000, is_shuffle=True):
    train_x,train_y=train_data[features1].values,train_data[label].values
...
def bulid_onetrain2(train_data, test,pred= features2,label= 'label',seed=1099,est=6000, is_shuffle=True):
    train_x,train_y=train_data[features2].values,train_data[label].values
    clf=LGBMRegressor( learning_rate=0.01,
    boosting_type = 'gbdt',
    objective = 'regression',
    n_estimators=est,
...
#result,clf=bulid_onetrain(train, test1,pred= features,label= 'label',est=6000,is_shuffle=True)


# In[62]:


result,clf=bulid_onetrain1(train_1, test_1,pred= features1,label= 'label',est=8000,is_shuffle=True)
print('--------------------------------------------------------------------------------')
result,clf=bulid_onetrain2(train, test_2,pred= features2,label= 'label',est=8000,is_shuffle=True)
print('----------------------------------------------------------------------------------')
test4 = pd.concat([test_1,test_2],axis=0)
