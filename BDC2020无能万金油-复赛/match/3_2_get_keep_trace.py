...
# 0-110  0-189  2-394  2-385
# test_list = ['FRLEH-CNYTN','HRRIJ-CNYTN','KWKWI-CNSHK','CLSAI-CNSHK']
...
#     print('开始判断是否为所需的历史路由')
    ...
    for i in test_list:
        if i[0] in TRANSPORT_TRACE and i[-1] in TRANSPORT_TRACE:# and i[-1] in TRANSPORT_TRACE:
            print(TRANSPORT_TRACE)
            if TRANSPORT_TRACE.index(i[0])<TRANSPORT_TRACE.index(i[-1]):
                if len(i) == 2:
                    trace_list.append(i)
                    do_later = True
                    long_trace.append(i)
                else:
                    index_1 = TRANSPORT_TRACE.index(i[0])                
                    index_2 = TRANSPORT_TRACE.index(i[-1])
                    if TRANSPORT_TRACE[index_1:index_2+1] == i:
                        do_later = True
                        trace_list.append(i)
                        long_trace.append(i)
    # %% 判断是否要截取路径
...
dff.to_csv('./data/test_data_normal_trace.csv',index = False)
